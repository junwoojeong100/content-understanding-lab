...
| 여러 문서 형식과 사용자 피드백 반영 | [초보자를 위한 피드백 루프 가이드](docs/feedback-loop-guide.md) |
...
1. `src/schema.py`가 문서에서 **무엇을 찾을지** 정합니다.
2. `src/extract_work_order.py`가 복잡한 결과에서 **실제 값만 꺼내기 쉽게** 정리합니다.
...
1. 같은 PDF로 RAW JSON, 정제 JSON, 현재 Excel을 만듭니다.
...
용어 설명, 여러 양식의 분석기 분리 기준, 문제 위치를 찾는 네 가지 질문, 실제 명령과 완료 체크리스트는
다음 문서에서 단계별로 설명합니다.
